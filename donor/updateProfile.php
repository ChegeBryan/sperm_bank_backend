...
$sql = "UPDATE profiles SET sperm_bank_id = ?,  name = ? , height = ? , birth_date = ?, ethnicity = ?, weight = ?, eye_color = ?, hair_color = ?, complexion = ?, education = ?, bloodtype = ?, interests = ?, hobbies = ? WHERE donor_id = ?";

if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("isdsssdssssssi", $sperm_bank, $param_name, $param_height, $param_birth, $param_ethnicity, $param_weight, $param_eye_color, $param_hair_color, $param_complexion, $param_education, $param_bloodtype, $param_interests, $param_hobbies, $param_id);
